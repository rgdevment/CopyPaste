...
#define LISTENER_PLUGIN(obj) \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), listener_plugin_get_type(), \
                              ListenerPlugin))
...
static void listener_plugin_handle_method_call(
    ListenerPlugin* self,
...
static void method_call_cb(FlMethodChannel* channel, FlMethodCall* method_call,
                           gpointer user_data) {
  ListenerPlugin* plugin = LISTENER_PLUGIN(user_data);
  listener_plugin_handle_method_call(plugin, method_call);
}

void listener_plugin_register_with_registrar(FlPluginRegistrar* registrar) {
  ListenerPlugin* plugin = LISTENER_PLUGIN(
      g_object_new(listener_plugin_get_type(), nullptr));
